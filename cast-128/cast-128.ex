defmodule Cast128 do
    ...
    def scheduleA() do
      [[[0x4, 0x0, 0xd, 0xf, 0xc, 0xe, 0x8],
        [0x5, 0x2, 0x10, 0x12, 0x11, 0x13, 0xa],
        [0x6, 0x3, 0x17, 0x16, 0x15, 0x14, 0x9],
        [0x7, 0x1, 0x1a, 0x19, 0x1b, 0x18, 0xb]
       ],

      [
        [0x0, 0x6, 0x15, 0x17, 0x14, 0x16, 0x10],
        [0x1, 0x4, 0x0, 0x2, 0x1, 0x3, 0x12],
        [0x2, 0x5, 0x7, 0x6, 0x5, 0x4, 0x11],
        [0x3, 0x7, 0xa, 0x9, 0xb, 0x8, 0x13]
       ],
    ...
    # Accessor function for sBox
    def sBoxAt(r, c) do
        at(at(sBox, r), c)
    end

    def keySchedule(key, rounds) do
      x = [6,7,4,5]
      roundLst = Enum.to_list 0..rounds-1

      get_t = fn(i) ->
          j = 4 * i
          at(key, j)     <<< 24 ||| at(key, j + 1) <<< 16 ||| 
          at(key, j + 2) <<< 8  ||| at(key, j + 3)
      end

      keyScheduleA = fn(r, j, t) ->
        a = at(at(scheduleA, r), j)
        
        calcW = fn(n) -> 
          sBoxAt(n + 2, (at(t, at(a, n) >>> 2) >>> 
                        (24 - 8*(at(a, n)  &&& 3))) &&& 0xff)
        end
        
        w = foldl [2,3,4,5], (at(t, at(a, 1))), &(&2 ^^^ calcW.(&1)) 
        w2 = w ^^^ sBoxAt(at(x, j), (at(t, at(a, 6) >>> 2) >>> 
                         (24 - 8 * (at(a, 6) &&& 3))) &&& 0xff)
        
        t2 = replace_at t, at(a, 0), w2
        t2
      end

      keyScheduleB = fn(r, j, t, k, ki) ->
        b = at(at(scheduleB, r), j)
        ...
        w2 = w  ^^^ sBoxAt(4 + j, (at(t, at(b, 4) >>> 2) >>> 
                        (24 - 8 * (at(b, 4) &&& 3))) &&& 0xff)
        k2 = replace_at k, ki, w2
        {k2, ki + 1}
      end 
      
      calcRoundA = fn(r, t) ->
        foldl [0,1,2,3], t, &(keyScheduleA.(r, &1, &2))

      end
      calcRoundB = fn(r, t, k, ki) ->
        foldl [0,1,2,3], {k, ki}, 
              &(case (&2) do
                  {k, ki} -> keyScheduleB.(r, &1, t, k, ki)
                end)
      end

      calcRound = fn(r, t, k, ki) ->
        t2 = calcRoundA.(r, t)
        {k2, ki2} = calcRoundB.(r, t2, k, ki)
        {t2, k2, ki2}
      end

      calcRounds = fn(t, k, ki) ->
        foldl roundLst, {t, k, ki},
              &(case (&2) do
                  {t, k, ki} -> calcRound.(&1, t, k, ki)
                end)
      end

      calcHalves = fn(t, k, ki) ->
          {t2, k2, ki2} = calcRounds.(t, k, ki)
          {_, k3, _} = calcRounds.(t2, k2, ki2)
          k3
      end 
      ...
      k2 = calcHalves.(t, k, ki)

      {masking, k3} = split k2, 16
      rotate = map k3, &(&1 &&& 0x1f)
      {masking, rotate} 
    end
    ...
    def encrypt(byteN, plainTxt, cypherTxt, mask, rot) do
        if byteN >= length plainTxt do
          cypherTxt
        else
          cypher8 = encrypt8(byteN, plainTxt, mask, rot)  
          encrypt(byteN + 8, plainTxt, cypherTxt ++ cypher8, mask, rot)    
        end
    end

    #Encrypts 64bit chunk of plaintxt
    def encrypt8(byteN, plainTxt, masking, rotate) do
        pTxtElem = fn(n) -> at(plainTxt, n) end
        ...
        l = pTxtElem.(byteN)    <<< 24 ||| pTxtElem.(byteN + 1) <<< 16 ||| 
            pTxtElem.(byteN + 2) <<< 8 ||| pTxtElem.(byteN + 3) 
